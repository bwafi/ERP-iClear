...
OWN-WORLD: Committed Modernize admin tokens only, consumed as resolved CSS vars (`--bs-primary`, `--bs-success`, `--bs-warning`, `--bs-danger`, `--bs-border-color`, `--bs-body-bg`, `--bs-secondary-color`) so Blue_Theme's `#0085db` primary and `#4bd08b` success and every dark-mode value come from the app itself. Debet = primary-tinted chip, Kredit = warning-tinted chip, danger reserved for Hapus alone. Iconify `solar:*` for chrome, Bootstrap Icons for row glyphs. Tabular-nums for every Rupiah figure and for IDs. Card geometry is the app's own: `--kk-surface: var(--bs-body-bg)` at 1.125rem (18px) radius with the single existing elevation `0 2px 6px rgba(37,83,185,.1)`, no gradients, no new fonts, no new palette. Surface must read as the same card stock as every other page — 2026-09-26 repair: `--kk-surface` resolved `var(--bs-card-bg)`, which is only declared inside `.card` (styles.css:5477), so outside a card the token was empty and the decks painted the bare canvas instead.
