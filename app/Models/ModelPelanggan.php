...
public function searchPelanggan($keyword, $limit = 20)
{
    ...
    {
        if ($per_bulan) {
            return $this->select("DATE_FORMAT(pelanggan.create_on, '%Y-%m') AS bulan, COUNT(DISTINCT pelanggan.id_pelanggan) AS total")->join('service', 'service.pelanggan_id_pelanggan = pelanggan.id_pelanggan')->where('pelanggan.deleted', '0')->where('service.status_service', 4)->groupBy('bulan')->orderBy('bulan', 'ASC')->findAll();
        }

        return $this->select('pelanggan.*, service.no_service')->join('service', 'service.pelanggan_id_pelanggan = pelanggan.id_pelanggan')->where('pelanggan.deleted', '0')->where('service.status_service', 4)->findAll();
    }
