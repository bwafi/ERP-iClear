...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white d-flex flex-wrap justify-content-between align-items-center gap-2">
        ...
        <?php if ($canWrite) : ?>
            <button type="button" class="btn btn-primary d-inline-flex align-items-center gap-1" onclick="openProspekModal()">
                <iconify-icon icon="solar:add-circle-bold" class="fs-6 lh-1"></iconify-icon><span class="lh-1">Tambah Prospek</span>
            </button>
        <?php endif; ?>
    </div>
    <div class="card-body">
            ...
            <form method="post" action="<?= base_url('marketing/leads/simpan') ?>" id="prospekForm">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <!-- Area Service (CLOSING wajib / terpilih) -->
                        <div id="pp_svc_area">
                            ...
                            <div id="pp_preview" class="d-none mt-2">
                                <div class="border rounded-3 p-3">
                                    ...
                                    <div class="row g-2 small">
                                        <div class="col-md-4"><span class="text-muted">Nama Customer:</span>
                                            <div><strong id="pp_svc_nama">-</strong></div>
                                        </div>
                                        <div class="col-md-3"><span class="text-muted">No HP:</span>
                                            <div><strong id="pp_svc_hp">-</strong></div>
                                        </div>
                                        <div class="col-md-2"><span class="text-muted">Unit:</span>
                                            <div><strong id="pp_svc_unit">-</strong></div>
                                        </div>
                                        <div class="col-md-3"><span class="text-muted">Tgl Service:</span>
                                            <div><strong id="pp_svc_tgl">-</strong></div>
                                        </div>
                                        ...
                                        <div class="col-md-4 d-none" id="pp_omset_row">
                                            <span class="text-muted">Omset:</span>
                                            <div class="fw-semibold" id="pp_svc_omset">-</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <!-- Area data manual (selain CLOSING & belum terhubung service) -->
                        ...
                        <div class="row g-2">
                            <div class="col-md-4" id="pp_tgl_booking_col">
                                <label class="form-label small text-muted mb-1">Tanggal Booking <span class="text-muted fst-italic">(opsional)</span></label>
                                <input type="date" name="tanggal_booking" id="pp_tgl_booking" class="form-control form-control-sm">
                            </div>
                            <div class="col-md-8">
                                <label class="form-label small text-muted mb-1">Catatan <span class="text-muted fst-italic">(opsional)</span></label>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="d-flex flex-column gap-2">
                            ...
                            $stDesc = [
                                ...
                                'CLOSING'  => 'Selesai — wajib pilih service selesai, omset otomatis (sum sub total).',
                        ...
                        <p class="small text-muted mt-3 mb-0">
                            ...
                            Untuk <strong>CLOSING</strong>, service selesai wajib dipilih agar omset terhitung otomatis (sum sub total); tanggal_won otomatis hari ini bila belum terisi.
                        </p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light" data-bs-dismiss="modal">Batal</button>
...
<script>
    ...
    var ppClosed = function() {
        return document.getElementById('pp_status').value === 'CLOSING';
    };
    var ppHasService = function() {
        return !!document.getElementById('pp_service_id').value;
    };
    var ppSvcData = null;

    function setText(id, v) {
        document.getElementById(id).textContent = v === null || v === undefined || v === '' ? '-' : v;
    }
    ...
    function syncProspekMode() {
        ...
        // Unit hanya dipakai utk non-CLOSING; CLOSING diambil dari service.
        document.getElementById('pp_unit_col').classList.toggle('d-none', closed);
        document.getElementById('pp_unit').required = !closed;

        // Tanggal booking tidak relevan saat CLOSING.
        document.getElementById('pp_tgl_booking_col').classList.toggle('d-none', closed);
        if (closed) {
            document.getElementById('pp_tgl_booking').value = '';
        }

        // Manual: hanya saat bukan CLOSING dan belum ada service terpilih.
        manualArea.classList.toggle('d-none', closed || hasSvc);
        preview.classList.toggle('d-none', !hasSvc);

        if (hasSvc) {
            hint.innerHTML = 'Detail prospek diambil dari service. Anda dapat menyesuaikan Status & Catatan.';
        } else if (closed) {
            hint.innerHTML = 'Status <strong>CLOSING</strong> wajib memilih <strong>Service</strong> <strong>SELESAI</strong> per tanggal prospek (semua cabang). Omset otomatis = sum sub total penjualan service.';
        } else {
            hint.innerHTML = 'Belum ada service? Isi <strong>Nama/Akun</strong> secara manual. Service boleh dipilih bila sudah tersedia (opsional).';
        }
    ...
    initServiceSearch({
        ...
        params: function() {
            // CLOSING → cari per tanggal prospek, SEMUA unit (cabang tampil di hasil).
            if (ppClosed()) {
                return {
                    s: 1,
                    sdate: document.getElementById('pp_tanggal').value || ''
                };
            }
            return {
                s: 0,
                unit_id: document.getElementById('pp_unit').value || 0
            };
        },
    ...
    document.getElementById('pp_status').addEventListener('change', function() {
        // Status berpindah: jika CLOSING tanpa service → submit terkunci; jika keluar CLOSING → omset tak relevan.
        syncProspekMode();
    });
        ...
        btn.addEventListener('click', function() {
            ...
            if (svcId > 0) {
                renderPreview({
                    ...
                    status_label: this.dataset.status === 'CLOSING' ? 'SELESAI' : 'PROSES',
